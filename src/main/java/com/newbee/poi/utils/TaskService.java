...
public class TaskService implements TaskDao {
    ...
    @Scheduled(fixedDelay = 15_000L, initialDelay = 60_000L)//initialDelay——spring启动后60秒开始执行，fixedDelay——每个15秒执行一次
    @Override
    public void patrol() {
        System.out.println("Async开始打印了..........");
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("Async打印结束..........");
    }

    @Async
    @Override
    public void myAsyncTask() {
        LOGGER.info("myAsync服务");
    }
    ...
    public Future<String> asyncMethodWithReturnType() {
        ...
        Future<String> future = new AsyncResult<>("String类型的返回结果哦，亲！");
        return future;
    }
}
